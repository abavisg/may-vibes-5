...
file_handler = logging.FileHandler(os.path.join(LOG_DIR, "poller_debug.log"))
file_handler.setLevel(logging_level) # Log according to environment variable

# Create a formatter and add it to the handlers
formatter = logging.Formatter("%(asctime)s [%(levelname)s] [poller] %(message)s")
...
logger.info(f"Poller service file logging level set to {logging.getLevelName(file_handler.level)}")
